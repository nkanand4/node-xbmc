...
function Main()
    ...
    'end while
     ...
     http.SetUrl("http://192.168.1.5:12480/pingcheck")
     http.SetMessagePort(httpport)
...
Function appEventLoop() As Void
    ...
    while true
        ...
        if msg <> invalid and type(msg)="roUniversalControlEvent" then
            ...
            if msg.GetInt() <> 999
               http.SetUrl("http://192.168.1.5:12480/xbmc/commands/"+msg.GetInt().ToStr())
               http.SetMessagePort(httpport)
               http.AsyncGetToString()
               print "http://192.168.1.5:12480/xbmc/commands/"+msg.GetInt().ToStr()
               'return 
            endif
            'http.SetUrl
            'http.SetUrl("http://binmonk.com/")
            'http.SetMessagePort(prot)
            'http.AsyncGetToString()
        end if
...
Function snkMakeLonger(app)
    ...
    return false 'Nitesh head.CheckCollision()<>invalid
...
Function snkTurn(app, newdx, newdy)
    ...
    if newhead.CheckCollision()<>invalid then return false 'Nitesh true
